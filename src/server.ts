...
const TOOLS = [
  ...
  {
    name: "run_crypto_scan",
    description:
      "Run an OUTWARD cryptographic discovery scan across an estate and get a " +
      "CycloneDX 1.6 CBOM. Unlike get_cbom (which inventories already-onboarded " +
      "integrations), this actively scans real assets: live TLS endpoints, X.509 " +
      "certificates (PEM) and dependency manifests (requirements.txt, package.json, " +
      "go.mod, pom.xml). Each discovered algorithm is classified for quantum " +
      "vulnerability and Harvest-Now-Decrypt-Later (HNDL) exposure, mapped to its " +
      "recommended PQC replacement (ML-KEM-768 / ML-DSA-65 / AES-256-GCM). Returns a " +
      "summary, an ordered remediation plan and a scan id. Private/internal endpoints " +
      "are refused by the SSRF guard.",
    inputSchema: {
      type: "object",
      properties: {
        name: { type: "string", description: "Optional label for this scan." },
        endpoints: {
          type: "array",
          description: "Public TLS endpoints to scan.",
          items: {
            type: "object",
            properties: {
              host: { type: "string", description: "Hostname or FQDN (no scheme)." },
              port: { type: "number", description: "TLS port (default 443)." },
              protected: {
                type: "boolean",
                description: "True if already behind the QuantaSeal PQC tunnel.",
              },
            },
            required: ["host"],
          },
        ...
        certificates: {
          type: "array",
          description: "PEM-encoded X.509 certificates to analyse.",
          items: {
            type: "object",
            properties: {
              label: { type: "string" },
              pem: { type: "string", description: "PEM certificate text." },
        ...
        manifests: {
          type: "array",
          description: "Dependency manifests or source snippets to pattern-scan.",
          items: {
            type: "object",
            properties: {
              filename: { type: "string", description: "e.g. requirements.txt, package.json." },
              content: { type: "string" },
              protected: { type: "boolean" },
            },
    ...
    annotations: { title: "Run Outward Crypto Discovery Scan", readOnlyHint: false },
  },
  {
    name: "get_crypto_scan",
    description:
      "Fetch a previously run outward crypto discovery scan by id, including its " +
      "summary, remediation plan and (optionally) the CycloneDX 1.6 CBOM document or " +
      "an ML-DSA-65 signed, regulator-admissible report.",
    inputSchema: {
      type: "object",
      properties: {
        scan_id: { type: "string", description: "The scan id returned by run_crypto_scan." },
        include: {
          type: "string",
          enum: ["summary", "cyclonedx", "report"],
          description:
            "summary (default) = detail; cyclonedx = the CBOM document; " +
            "report = ML-DSA-65 signed report.",
        },
      },
      required: ["scan_id"],
    },
    annotations: { title: "Get Crypto Discovery Scan", readOnlyHint: true },
  },
] as const;
...
async function handleRunCryptoScan(input: ToolInput): Promise<string> {
  const body: Record<string, unknown> = {};
  if (input.name) body.name = input.name;
  body.endpoints = Array.isArray(input.endpoints) ? input.endpoints : [];
  body.certificates = Array.isArray(input.certificates) ? input.certificates : [];
  body.manifests = Array.isArray(input.manifests) ? input.manifests : [];
  const data = await call<Record<string, unknown>>(
    "POST",
    "/api/v2/security/crypto-scan",
    body,
  );
  return fmt(data);
}

async function handleGetCryptoScan(input: ToolInput): Promise<string> {
  const scanId = String(input.scan_id ?? "");
  const include = (input.include as string) || "summary";
  let path = `/api/v2/security/crypto-scan/${encodeURIComponent(scanId)}`;
  if (include === "cyclonedx") path += "/cyclonedx";
  else if (include === "report") path += "/report";
...
const HANDLERS: Record<string, (input: ToolInput) => Promise<string>> = {
  ...
  run_crypto_scan: handleRunCryptoScan,
  get_crypto_scan: handleGetCryptoScan,
};
  ...
  const server = new Server(
    {
      ...
      description:
        ...
        "and check SOC 2, HIPAA, GDPR, PCI DSS compliance — 23 tools, NIST FIPS 203/204/205.",
